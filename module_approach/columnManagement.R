  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$sticky, {
      showModal(modalDialog(
        ...
        div(
          style = "margin-bottom: 15px",
          actionButton(session$ns("selectAlls"), "Select All"),
          actionButton(session$ns("unselectAlls"), "Unselect All")
        ),
        div(
          style = "max-height: 400px; overflow-y: auto;",
          checkboxGroupInput(
    ...
    # Select/Unselect All handlers
    observeEvent(input$selectAlls, {
      updateCheckboxGroupInput(session,
                               inputId = "columns_vars_s",
                               choices = names(filtered_data()),
                               selected = names(filtered_data()))
    })
    
    observeEvent(input$unselectAlls, {
      updateCheckboxGroupInput(session,
                               inputId = "columns_vars_s",
                               choices = names(filtered_data()),
                               selected = character(0))
    ...
    observeEvent(input$columns_vars_s, {
      req(selected_file())
      if(is.null(input$unselectAlls)){
        current_selections_s[[selected_file()]] <- NULL
      }else{
        current_selections_s <- selected_vars_s()
